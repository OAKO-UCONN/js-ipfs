...
describe('pin', function () {
  ...
  async function isPinnedWithType (path, type) {
    try {
      for await (const _ of pin.query(path, { type })) { // eslint-disable-line no-unused-vars
        return true
      }
      return false
    } catch (err) {
  ...
  describe('ls', function () {
    ...
    it('should list pins of a particular CID', async () => {
      const out = await all(pin.query(pins.mercuryDir))
    ...
    it('should list direct pins for CID', async () => {
      const out = await all(pin.query(pins.mercuryDir, { type: 'direct' }))
    ...
    it('should list direct pins for path', async () => {
      const out = await all(pin.query(`/ipfs/${pins.root}/mercury/`, { type: 'direct' }))
    ...
    it('should list direct pins for path (no match)', () => {
      return expect(all(pin.query(`/ipfs/${pins.root}/mercury/wiki.md`, { type: 'direct' })))
        .to.eventually.be.rejected()
    })

    it('should list direct pins for CID (no match)', () => {
      return expect(all(pin.query(pins.root, { type: 'direct' })))
        .to.eventually.be.rejected()
    })

    it('should list recursive pins for CID', async () => {
      const out = await all(pin.query(pins.root, { type: 'recursive' }))
    ...
    it('should list recursive pins for CID (no match)', () => {
      return expect(all(pin.query(pins.mercuryDir, { type: 'recursive' })))
        .to.eventually.be.rejected()
    })

    it('should list indirect pins for CID', async () => {
      const out = await all(pin.query(pins.solarWiki, { type: 'indirect' }))
    ...
    it('should list indirect pins for CID (no match)', () => {
      return expect(all(pin.query(pins.root, { type: 'indirect' })))
        .to.eventually.be.rejected()
    })
  })
